...
import copy
import numpy as np
from myenv.OthelloFunction import check_finished, do_action,get_onehot,get_reward,get_possible_actions
...
class OthelloRandomEnv(gym.Env):
    ...
    def _reset(self):
        ...
        if random.random() >= 0.5:
            ...
            action = self.get_random_action(self.board, self.env_hand)
    ...
    def get_random_action(self,board,hand):
        action_list = get_possible_actions(board,hand)
        if action_list == []:
            return 64
        else:
